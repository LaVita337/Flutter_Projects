      ...
      body: Center(
        child: Column(
          ...
          children: [
                  ...
                  SizedBox(height: 15),  // height 10 -> 15 으로 변경 진영
                      Text(
                        ...
                        style: TextStyle(
                          ...
                          fontSize: 17.0,   // 23 -> 17로 변경 진영
                        ),
                      ),
                ],
              ),
            ),
            SizedBox(height: 110), // 20 -> 110
            Container(
              width: 330,
              height: 70, // 이전 색상과 모양으로 맞춘 Container
              decoration: BoxDecoration(
                color: globals.isfirstCorrect ? Color(0xFF81c784) : Color(0xFFf28b82), // 이전에 사용한 빨간색
                borderRadius: BorderRadius.circular(20), // 둥근 모서리
              ),
              alignment: Alignment.center, // Container 내부의 모든 위젯을 중앙에 위치시킵니다.
              child: Text(
                globals.isfirstCorrect ? '정답 : "${globals.selectedAnswer}"' : '오답 : "${globals.selectedAnswer}"',
                style: TextStyle(
            ...
            SizedBox(height: 30), // 10 -> 30
            Container(
              width: 330,
              height: 65, // 이전 색상과 모양으로 맞춘 Container
              decoration: BoxDecoration(
                color: globals.isSecondOptionCorrect ? Color(0xFF81c784) : Color(0xFFf28b82), // 이전에 사용한 초록색
                borderRadius: BorderRadius.circular(20), // 둥근 모서리
              ),
              alignment: Alignment.center, // Container 내부의 모든 위젯을 중앙에 위치시킵니다.
              child: Text(
                globals.isSecondOptionCorrect ? '정답 : "${globals.selectedAnswer2}"' : '오답 : "${globals.selectedAnswer2}"',
                style: TextStyle(
                  color: Color(0xfffff6f6),
                  fontWeight: FontWeight.w600,
                  fontFamily: "Poppins",
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                  if (openCount >= 1) {
                    SystemNavigator.pop(); // openCount가 1이상('다음'버튼 2회 터치 이상)이면 앱을 종료합니다.
                  } else {
                    // LockScreen2으로 이동합니다.
